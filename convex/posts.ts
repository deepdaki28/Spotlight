    ...
    handler: async (ctx) => {
        ...
        const postsWithInfo = await Promise.all(
            posts.map(async (post) => {
                ...
                const like = await ctx.db.query("likes").withIndex("by_user_and_post", (q) => q.eq("userId", currentUser._id)
                    .eq("postId", post._id)).first();

                const bookmark = await ctx.db.query("bookmarks").withIndex("by_user_and_post", (q) => q.eq("userId", currentUser._id)
                .eq("postId", post._id)).first();
...
export const toggleLike = mutation({
    ...
    handler:async(ctx,args) => {
        ...
        const exsisting = await ctx.db.query("likes").withIndex("by_user_and_post", (q) => q.eq("userId", currentUser._id)
        ...
        if(exsisting){
            await ctx.db.delete(exsisting._id)
